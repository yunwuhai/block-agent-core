...
describe("Runtime runner", () => {
  ...
  it("persists and restores slots across continuation runs", async () => {
    // Run 1: set a slot
    ...
    // Save slots to disk like executeRun does
    const { writeFile, readFile } = await import("node:fs/promises");
    const { join } = await import("node:path");
    const slotsPath = join(r1.runDir.dir, "slots.json");
    await writeFile(slotsPath, JSON.stringify(serializeSlots()) + "\n", "utf-8");

    // Verify slots.json exists
    const raw = await readFile(slotsPath, "utf-8");
    const parsed = JSON.parse(raw);
    expect(parsed.slots["test-slot-x"].content).toBe("persisted content");
    ...
    // Restore from disk (as executeRun would do on continuation)
    deserializeSlots(parsed);
    expect(listSlots().has("test-slot-x")).toBe(true);
    expect(listSlots().get("test-slot-x")?.content).toBe("persisted content");
  });
});
