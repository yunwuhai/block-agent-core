...
efficiency-subagent 是 PI Coding Agent 的一个基于 profile 的子代理插件。它可以让你调用受控的子代理，在沙箱权限、hook 脚本、动态 prompt 注入和持久 session 下运行。每次运行都会产出结构化的 handoff 文档，使后续调用可以从中断处继续。插件完全运行在宿主代理的扩展系统内部，不自行管理 LLM 调用或沙箱进程。
...
**后端 Computation** 是最大的一层，共 7 个模块。包含策略引擎、完整的 prompt 注册表管线（类型、引擎、组装器）、prompt-engine（slot/placeholder 管理）、hook-system（脚本运行器和输出注入）以及 hook-scripts（用户编写的可执行脚本）。
...
→ 合并策略 → before_agent hooks → 构建 prompt（registry + slots + placeholders）
→ 动作循环（每个动作：策略检查 → before_tool hook → 工具 → after_tool hook）
→ after_agent hooks → 构建 transcript → 写入 handoff → 持久化存储 → TUI 渲染
```
...
插件注册了一个工具：`efficiency_subagent`。使用以下参数调用：
...
当你用 `profile` 和 `task` 调用时，系统会加载指定 profile 的 YAML 前置元数据来确定：可用工具有哪些、各阶段运行哪些 hooks、注册哪些 placeholder 值、激活哪些 registry 条目、使用哪个 system prompt 模板。`task` 字符串会成为发送给子代理的用户消息。
...
**功能：** 定义所有 Zod schema（ToolParams、ProfileFrontmatter、HooksConfig、RegistryEntry、ProjectPolicy），在系统边界校验输入参数。
...
**何时交互：** 创建或调试 profile 定义时。关键函数：`listProfiles()`、`loadProfile(name)`。
...
**何时交互：** 编写注入内容的 hook 脚本，或需要理解 prompt 组装机制时。关键导出：`setSlot()`、`pushSlot()`、`popSlot()`、`setOnceSlot()`、`registerPlaceholder()`、`renderPromptWithRegistry()`、`serializeSlots()`、`deserializeSlots()`。
...
**功能：** 17 阶段中心编排器。执行运行：加载 profile、合并策略、调度 hooks、运行带重试逻辑的动作循环、构建 transcript 和 handoff、持久化状态。同时处理 run ID 解析和续跑一致性检查。
...
Hooks 是用户编写的 TypeScript 脚本，在四个生命周期节点执行：`before_agent`、`after_agent`、`before_tool`、`after_tool`。脚本位于 `hooks/scripts/`，在运行时动态导入。

### 生命周期阶段

- **before_agent**：在 prompt 构建和动作循环开始前运行。脚本可通过返回 `allowed: false` 阻断执行。
- **after_agent**：在动作循环完成后、transcript 和 handoff 生成前运行。
- **before_tool**：在每次工具调用前运行。可通过 `modifiedArgs` 修改工具参数。
- **after_tool**：在每次工具完成后运行。接收工具结果。

### 脚本契约

每个 hook 脚本导出一个函数：`(ctx: HookContext) => Promise<HookResult>`。上下文提供了运行目录路径、当前阶段、工具名/参数（对工具 hooks）和运行元数据。结果可包含 slot 内容、修改后的参数和可选的 session 消息。

### 输出流入 Slot 的方式

Hook 输出通过两种注入策略流入 prompt：

1. **直接 slot 注入**：`injectHookOutputAsSlot()` 调用 prompt-engine 的 `setSlot()`，使 hook 的输出作为命名 slot 可用，按优先级插入到 prompt 前部。
2. **基于 Registry 的注入**：`registerHookOutput()` 创建临时 registry 条目并通过 orchestrator 调度注入。这种方式更丰富，支持优先级排序和频率控制。

Hook 脚本还可以产出 `sessionMessage` 结果，这些结果会被追加到代理的对话历史中。
...
1. **创建 profile**：在 `.profiles/worker.md` 中使用 YAML 前置元数据定义工具、hooks、placeholder、registry 条目和 prompt 正文。
...
一个 profile markdown 文件有两部分：YAML 前置元数据（位于 `---` 标记之间）和 markdown 正文。前置元数据声明子代理的 system prompt、工具列表、hook 脚本、placeholder 值和 registry 条目。正文作为附加上下文。结构示例：
...
一个 hook 脚本导出一个异步函数。它接收 `HookContext` 并返回 `HookResult`。脚本可以通过 `Bun.spawnSync` 执行 shell 命令、读取文件和格式化输出。契约示例：

```typescript
export default async function(ctx: HookContext): Promise<HookResult> {
  // 执行工作，返回包含可选 slotContent、modifiedArgs、sessionMessage 的结果
  return { allowed: true };
}
```
...
插件不附带任何内置 profile。所有 profile 必须由用户在 `.profiles/*.md` 中创建。`hooks/scripts/` 中的 `_example.ts` hook 脚本展示了契约但不用于生产环境。
