...
- `SessionToolCallRecord` 加 `startedAt` / `finishedAt`

### T1-6 错误分类 ★★ $

当前 `error: boolean` 无法区分可重试和不可重试。

- `SessionToolCallRecord` 加 `errorCode: string` 和 `errorCategory: "transient" | "terminal" | "resource_exhausted"`
...
- `SessionToolCallRecord` 加 `idempotencyKey`
- `appendSessionToolCall` 检测重复 key 返回已有记录
...
| # | 位置 | 描述 |
|---|------|------|
| 1 | `session-runtime.ts:87` | `toolCallSeq` 不存在于 `SessionMessageRecord` |
| 2 | `session-runtime.ts:93` | `fileCallSeq` 不存在于 `SessionMessageRecord` |
