...
| `handleLoad(params, ctx)` | 37-70 | 主 handler — 查 callRecord → 解析 refs → 拼装提示词 |

### `handleLoad` 内部流程

| 步骤 | 行号 | 说明 |
|------|------|------|
| 查找调用记录 | 42-49 | 按 `recipeId` 或取最后一条 |
| 解析 ref 内容 | 51-59 | 遍历所有 zone 的 refs，去重解析 |
| 拼装提示词 | 64-66 | 用解析后的内容和 resolver 调用 `buildPrompt` |
