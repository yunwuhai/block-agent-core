...
**Imports from:** `backend/computation/policy/`, `backend/computation/prompt/engine.ts`, and `backend/storage/`.
...
|---|---|---|---|
| `ToolInteractionResult` | interface | 8–11 | Return contract for tool execution: `output` string and `blocked` boolean. |
| `executeWithRetry` | async function | 13–56 | Wraps `simulateToolInteraction()` with up to 2 retries and exponential backoff. Retries timeout/network/ECONNREFUSED errors only. |
| `simulateToolInteraction` | async function | 58–135 | Single-tool pipeline: abort check, policy `evaluate()`, simulated call/result JSONL logging, slot mutation event logging. |
...
3. Tool call simulation appends call/result entries to `tools.jsonl` and records `tool_call`/`tool_result` events.
...
|---|---|---|---|
| `isoNow()` | function | 137–139 | Returns the current ISO timestamp for durable tool/event logs. |
