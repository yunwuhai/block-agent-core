...
**Test suite:** `"Efficiency Subagent extension"` (line 5–65)
...
| L38 | Execute rejects empty/invalid params | Calling `execute("", {})` returns `{ isError: true }` |
| L52 | Render call formats profile + task into output | `render({ profile: "worker", task: "fix bugs" })` returns a string containing both values |
...
- Does not test actual subagent execution (runner, hooks, permissions, slots) — those live in their own test files.
