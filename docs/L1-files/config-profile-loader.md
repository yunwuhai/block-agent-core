...
### `loadProfile(cwd: string, profileName: string): Promise<ProfileDefinition>` (line 422–450)
...
| `extractFrontmatter(content)` | 376–405 | Splits markdown content into YAML frontmatter (between `---` delimiters) and body |
| `resolveProfileDir(cwd)` | 411–414 | Internal helper resolving `.profiles/` relative to the working directory |
